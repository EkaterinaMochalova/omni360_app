...
/// Загруженное расписание кампании.
///
/// Пустой `slots` — это «ограничений по времени нет, вещаем круглые сутки», а
/// не «данных нет». Различать обязательно: если считать пустое расписание
/// неизвестным, уведомления замолчат навсегда (у кампаний без таргетинга по
/// времени `timeSettings` приходит пустым массивом), а если считать
/// неизвестное расписание круглосуточным — уведомления придут ночью.
/// Отсутствие самого объекта (null) означает «загрузить не удалось».
class BroadcastSchedule {
  final List<TimeSlot> slots;

  const BroadcastSchedule(this.slots);

  /// Есть ли ограничения по времени. false — кампания идёт круглые сутки.
  bool get hasRestrictions => slots.isNotEmpty;
}
